 ...
 *
 * Six standard reports (Pipeline / Revenue / Customers / Comms /
 * Calendar / Battery), an ad-hoc range picker (today / 7d / 30d / 90d),
 * and an AI insight card that turns the bundle into a Spanish narrative
 * via the active agent.
 *
 ...
 * left border, the 6 report cards use a 1/2/3-col responsive grid
 * (peer reports → uniform grid IS correct here; variation lives inside
 * each card via different content shapes).
 */
export default async function ReportesPage({
  ...
  const bundle = await buildReportsBundle(db, primary.tenant.id, range);
...
function PipelineCard({ bundle }: { bundle: ReportsBundle }) {
  ...
  return (
    <ReportCard eyebrow="Ventas" title="Pipeline" href="/workspace/ventas">
      <Stat
